...
def g(fake_imgs,keep_prob,reuse=tf.AUTO_REUSE):
...
def d(raf_img,reuse=tf.AUTO_REUSE):
...
x = tf.placeholder(tf.float32,[None,100,28*28])
...
t = 1
for i in outputs[:-1]:
    g_z = np.random.normal(0, 1, size=(batch_size, 28 * 28))
    innum = tf.concat([g_z, i], 1)
    if t == 1:
        gfake = g(innum,keep_prob)
        dreal = d(tf.concat([x[t],i],1))
        t += 1
    else:
        gfake = g(innum,keep_prob,True)
        dreal = d(tf.concat([x[t],i],1),True)
    dfake = d(tf.concat([gfake,i],1),True)
...
var_lstm = [var for var in tf.trainable_variables() if var.name.startswith('RNN')]
train_g = tf.train.AdamOptimizer(lr).minimize(lossgall,var_list=[var_g,var_lstm])
train_d = tf.train.AdamOptimizer(lr).minimize(lossdall,var_list=var_d)

cellpreout = [x[0]]
init_states = mlstm_cell.zero_state(batch_size, dtype=tf.float32)
states = init_states
with tf.variable_scope('RNN',reuse=True):
    for timestep in range(timestep_size):
        (cellout, states) = mlstm_cell(cellpreout[-1], states)
        g_z = np.random.normal(0, 1, size=(batch_size, 28 * 28))
        cellout = tf.concat([g_z,cellout],1)
        prefake = g(cellout,keep_prob,reuse=tf.AUTO_REUSE)
