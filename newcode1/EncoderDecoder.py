...
decoderfake = decoderModel(g_fake,keep_pro)
d2outreal = d2(encoder_x)
...
with tf.Session() as sess:
    ...
    for i in range(epoch+1):
        ...
        _,lossg2 = sess.run([train_e2d,lossen2de2],feed_dict={x:x_train,y:y_train,z:gz,keep_pro:0.5})
        if i % 500 == 0:
            t2 = time.time()
            print("epoch:{0}  loss_en2d:{4} loss_d:{1}     loss_g:{2} loss_d2{5} loss_g2 time:{3}".format(i,loss_d,loss_g,t2-t1,loss_en2de,
                                                                                                   loss_d2,lossen2de2))
            t1 = t2
        if i % 5000 == 0:
