...
cosine = tf.reduce_mean(tf.divide(tf.reduce_sum(tf.multiply(g_fake,g_real),1),tf.reduce_mean(tf.multiply(g_fake,g_fake),1)*
                   tf.reduce_mean(tf.multiply(g_real,g_real))))
...
lossg = -tf.reduce_mean((doutfake))+cosine
