...
h2 = con2d(h1,w_shape=[5,5,32,32],b_shape=[32],h_dim=2,reuse=False)
h2 = tf.reshape(h2,[-1,7*7*32])
f2 = fcont(h2,keep_drop=keep_drop)
loss = -tf.reduce_mean(y*tf.log(f2))
...
with tf.Session() as sess:
    ...
    acc = tf.equal(tf.argmax(y, 1), tf.argmax(f2, 1))
